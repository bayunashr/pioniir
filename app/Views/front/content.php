...
<div class="container mt-4 mb-10">
   <div class="bg-white px-1 px-sm-5 py-8 rounded">
      <div class="px-3 px-md-8 px-sm-15">
         <a href="<?= base_url('content/'.$content['creatorAlias']) ?>" class="fw-bolder hover fs-16"><i class="uil uil-angle-left-b"></i> Kembali</a>
      </div>
      <h1 class="display-5 text-center mt-10 mb-2"><?= $content['contentTitle'] ?></h1>
      <ul class="text-center post-meta mb-5">
         <li class="post-date fs-13"><i class="uil uil-calendar-alt"></i><span> <?= date('d F Y', strtotime($content['createdAt'])); ?></span></li>
         <li class="post-comments fs-13"><i class="uil uil-comment"></i> <?= count($comment) ?><span> Comments</span></li>
         <li class="post-likes fs-13"><i class="uil uil-heart-alt"></i> <span id="totalLike"><?= $content['contentLike'] ?></span><span> Love</span></li>
      </ul>
      <div class="content-value mt-10 px-3 px-md-8 px-sm-15">
         <figure class="mb-10"><img src="<?= base_url() ?>assets/uploads/thumbnail/<?= $content['contentPreview'] ?>" /></figure>
         <div>
            <?= $content['contentValue'] ?>
         </div>
         <a href="<?= $content['contentDownload'] ?>" class="btn btn-sm btn-yellow rounded-pill"><i class="uil uil-external-link-alt"></i> &nbsp; External Content</a>
         <div class="d-flex justify-content-between mb-5 mt-5">
            ...
            <button class="btn btn-sm <?= ($cekLove == null) ? 'btn-outline-red' : 'btn-red' ?>  rounded-pill btn-icon btn-icon-start love-button" data-content-id="<?= $content['contentId'] ?>" data-love-status="<?= ($cekLove == null) ? 'love' : 'unlove' ?>"><i class="uil uil-heart"></i>
               Loves</button>
            <?php endif?>
         </div>
   ...
   if (loveStatus === 'love') {
      $.ajax({
         ...
         success: function(response) {
            if (response.success) {
               $('.love-button[data-content-id="' + contentId + '"]').removeClass('btn-outline-red').addClass('btn-red');
               $('.love-button[data-content-id="' + contentId + '"]').attr('data-love-status', 'unlove');
      ...
      $.ajax({
         ...
         success: function(response) {
            if (response.success) {
               $('.love-button[data-content-id="' + contentId + '"]').removeClass('btn-red').addClass('btn-outline-red');
               $('.love-button[data-content-id="' + contentId + '"]').attr('data-love-status', 'love');
               var updatedLikes = currentLikes - 1;
               document.getElementById('totalLike').innerText = updatedLikes;
            }
         },
